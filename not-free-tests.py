...
class TestYourWebserver(unittest.TestCase):
    ...
    def test_hardcode(self):
        ...
        # self.assertTrue( req.info().get_content_type() == "text/html", ("1. Bad mimetype for html! %s" % req.info().get_content_type()))
        url = self.baseurl + "/hardcode/"
        req = request.urlopen(url, None, 3)
        self.assertTrue( req.getcode()  == 200 , "200 OK Not FOUND! Hardcoding? /hardcode/")
        self.assertTrue( req.info().get_content_type() == "text/html", ("2. Bad mimetype for html! %s" % req.info().get_content_type()))
